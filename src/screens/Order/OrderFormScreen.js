...
import { View, StyleSheet, TouchableOpacity, Animated, Text, TextInput, Easing, Modal, ScrollView } from 'react-native';
...
import { userActivity, calculateBill, findCustomerByCustomerId, requestPrioritiesForDropDown, shippingAddressByCustomer, create, getDetailsByCustomerOrderId, cencelOrderOrQuotation, validateQuantityBeforeSubmit, submit, getCustomerAccountDetailByCustomerId, submitForClient, getById, postSaleAndSaleReturnData, getQuotationById, approveQuotation, createTransactionEntry } from '../../services/drawerService';
...
const OrderFormScreen = ({ navigation, route }) => {
    ...
    const setScreenFieldsDataApi = async () => {
        try {
            const response = await setScreenFieldsData(fields, "M-SPS", "S-CUSTPMER-INQUIRY-ORDER-FORM", { 'langid': currentLanguage == 'ar' ? 2 : 1 });
            setFields(response);
            // dispatch(resetLoading());
        } catch (error) {
            setFields(true);
    ...
    const DetailsByCustomerOrderId = async () => {
        try {
            ...
            if (resData && (resData.code == 201 || resData.code == 200) && resData?.result) {
                ...
                if (data.status == 'SUBMITTED' || data.status == 'CANCEL' || data.status == 'APPROVED' || status == 'DELIVERED')
                ...
                resData.result?.details.length && resData.result?.details.map((x, index) => {
                    ...
                    } else if (data.status == 'SUBMITTED' || data.status == 'CANCEL' || data.status == 'APPROVED' || status == 'DELIVERED') {
                        let obj = {
                            ...x,
                            key: x.id + index + rand,
                            showHeader: false,
    ...
    const createQuotation = async (event) => {
        try {
            ...
            if (response.code == 201 || response?.code == 200) {
                if (event == 'save') {
                    ...
                    if (data && data.code == 201) {
                        saveUserActivity(data?.result?.id, null, 'ORDER SUBMITTED');
            ...
            //previous Function for saving and updating 
            ...
            // }
            // Toast.show({
            //     type: resdata.code == 201 ? 'success' : 'error', // 'error', 'info' can also be used
            //     text1: resdata?.btiMessage?.message,
            //     text2: resdata.code == 201 ? t('success') : t('error'),
            // });
            // dispatch(resetLoading());
    ...
    const saveUserActivity = async (referenceId, activeItem, actionType) => {
        try {
            ...
            else if (actionType == 'ORDER SUBMITTED') {
    ...
    // const saveUserActivity = async (activeItem) => {
    //     try {
    //         const headers = await getHeader();
    //         const body = {
    //             "customerNumber": userProfile.customerNo,
    ...
    //             // this.userActivityResponse = data.result;
    //         } else {
    //             return null;
    //         }
    ...
    return (
        <>
            <CustomHeader navigation={navigation} headerTitle={!id ? fields?.['CREATE_ORDER']?.fieldValue || 'Create Order' : fields?.['EDIT_ORDER']?.fieldValue || 'Edit Order'} />
            ...
            {(status != 'SUBMITTED' && status != 'CANCEL' && status != 'APPROVED' && status != 'DELIVERED') && <View style={styles.container}>
                <ItemFilter
            ...
            {(status == 'SUBMITTED' || status == 'CANCEL' || status == 'APPROVED' || status == 'DELIVERED') && <ScrollView style={{ backgroundColor: colors.primary, borderRadius: 5, flex: 1 }}>
                ...
                <View style={{ gap: 7, marginBottom: 30, marginHorizontal: 25, justifyContent: 'center' }}>
                    ...
                    <Button onPress={() => navigation.navigate('AllOrderScreen')}
